...
**Solution:** A freshness-checking script and a GitHub Actions workflow template, both generated by `/unslop:init`.

**Why `orchestrator.py build-order` cannot serve as the check:** `build-order` (orchestrator.py:249-256) only emits a topologically sorted list of specs and exits. It never reads managed-file headers, mtimes, or hashes. A hook that runs `python orchestrator.py build-order .` will pass in repos with stale generated files, giving teams a false sense that freshness is being enforced.

**What's actually needed: a dedicated `check-freshness` command** in `orchestrator.py` (or a standalone `scripts/check-freshness.py`) that:

1. Discovers all `*.spec.md` files (reuses `discover_files`)
2. For each spec, derives the managed file path (strips `.spec.md`)
3. Reads the managed file's `@unslop-managed` header and extracts `spec-hash` and `output-hash` (the dual-hash format from Gap 2)
4. Computes current spec hash and current output hash
5. Classifies each file as fresh, stale, or modified — the same logic the `status` command uses, but as deterministic Python rather than a prompt-driven skill
6. Exits non-zero if any file is stale or modified, printing the offending files

This is the same classification logic described in the `status` command (status.md:34-39), but implemented as executable code so it can run in hooks and CI without an LLM.

**`scripts/check-freshness.py` sketch:**
```python
#!/usr/bin/env python3
"""Exit non-zero if any managed file is stale or modified."""
import hashlib, re, sys, pathlib

def spec_hash(path: pathlib.Path) -> str:
    return hashlib.sha256(path.read_bytes()).hexdigest()[:12]

def parse_header(path: pathlib.Path) -> tuple[str | None, str | None]:
    """Extract spec-hash and output-hash from the @unslop-managed header."""
    text = path.read_text()
    m = re.search(r'spec-hash:(\w+)', text)
    s = m.group(1) if m else None
    m = re.search(r'output-hash:(\w+)', text)
    o = m.group(1) if m else None
    return s, o

def output_hash(path: pathlib.Path) -> str:
    """Hash managed file content below the header."""
    lines = path.read_text().splitlines(keepends=True)
    # Skip header lines (lines starting with comment + @unslop or spec-hash/output-hash)
    body_start = 0
    for i, line in enumerate(lines):
        if 'spec-hash:' in line or 'output-hash:' in line or '@unslop-managed' in line:
            body_start = i + 1
        else:
            break
...
# ... discover specs, classify, exit 1 if any non-fresh
```

**Pre-push hook (generated by `/unslop:init`):**
```bash
#!/bin/sh
python3 "$(git rev-parse --show-toplevel)/unslop/scripts/check-freshness.py" \
  ...
  check:
    ...
    steps:
      ...
      - name: Check managed file freshness
        run: python unslop/scripts/check-freshness.py .
      - name: Run tests
        run: <test command from config.json>
```

**What's needed:**
- New script: `scripts/check-freshness.py` implementing the dual-hash classification from Gap 2 as deterministic Python
- Update `init` command to generate a pre-push hook that calls `check-freshness.py`
- Later: GitHub Actions workflow template in `scripts/`
- The `status` command and `check-freshness.py` share classification logic — factor the hash parsing/comparison into a shared module or keep them independent (the script is ~50 lines; duplication is acceptable at this scale)
